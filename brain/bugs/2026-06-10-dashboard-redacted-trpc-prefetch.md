...
- 2026-06-11: Matched the GND internal API pattern for dashboard tRPC. `@afterservice/api/internal-api` now exports Next route handlers backed by the shared Hono API app, and the dashboard `/api/trpc` route re-exports those handlers instead of duplicating `fetchRequestHandler` locally.
